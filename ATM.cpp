...
static int SelectAccount(const BankSystem& bank, const std::string& title) {
    ...
    int pick = ReadIntInRange("Choose account: ", 1, static_cast<int>(accs.size()));
    return pick - 1;
}
...
int main() {
    ...
    // Security measure: limit login attempts
    ...
    while (true) {
        ...
        // Login loop
        while (!bank.IsLoggedIn() && attempts < maxAttempts) {
            std::string id = ReadLine("Enter User ID: ");
            std::string pin = ReadLine("Enter PIN: "); // Do not echo PIN elsewhere
        ...
        while (session) {
            ...
            switch (choice) {
                ...
                case 4: {
                    ...
                    if (to < 0) break;
                    if (from == to) {
                        std::cout << "Transfer canceled: choose different accounts.\n\n";
                        break;
                    }
